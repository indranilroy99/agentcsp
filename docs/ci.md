...
Use that summary to explain whether a gate failed because of severity thresholds, new findings, expired suppressions, or diagnostics. It also includes bounded blocker ID lists for severity-gated findings, expired suppressions, and diagnostics so CI systems can link directly to the relevant AgentCSP records without reimplementing gate logic. The summary does not include raw evidence snippets or secret values.
