...
The summary also includes `total_active_findings_considered`, `max_actions`, `omitted_actions`, and `truncated` so consumers can tell when the concise action queue hides lower-priority active findings. It also includes `new_actions`, `existing_actions`, and `by_owner`, a deterministic owner rollup with action count, highest severity, and max risk score. Owner hints are routing hints, not access-control decisions.
