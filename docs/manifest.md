...
Source-defined tool handlers can also emit redacted clipboard browser-automation bridge authority through `clipboard_browser_automation_bridge`, `clipboard_read`, `browser_automation`, `tainted_browser_automation_target`, and the corresponding `handler_*` fields when parsed MCP SDK or agent-framework tools read browser or desktop clipboard material and paste copied text into authenticated browser automation using runtime browser credentials and caller-selected targets or selectors. Handler bodies, clipboard helper calls, copied text, browser/page/driver calls, target URLs, selectors, pasted values, derived clipboard variables, and return strings remain redacted.

Source-defined tool handlers can also emit redacted clipboard artifact bridge authority through `clipboard_artifact_bridge`, `clipboard_read`, `artifact_export`, `tainted_artifact_export_payload`, `public_artifact_destination`, and the corresponding `handler_*` fields when parsed MCP SDK or agent-framework tools read browser or desktop clipboard material and export copied text into public or shareable artifact storage using runtime credentials and caller-selected artifact destinations. Handler bodies, clipboard helper calls, copied text, artifact storage calls, bucket names, object keys, public links, artifact payloads, derived clipboard variables, and return strings remain redacted.
...
Source-defined tool handlers can also emit redacted visual-context prompt-registry bridge authority through `visual_context_prompt_registry_bridge`, `visual_context_capture`, `browser_automation`, `tainted_browser_automation_target`, `prompt_registry_write`, `tainted_prompt_registry_payload`, `tainted_prompt_registry_selector`, and the corresponding `handler_*` fields when parsed MCP SDK or agent-framework tools capture authenticated browser screenshots, screen state, or OCR context from caller-selected targets and publish that visual context into prompt or instruction registries using runtime credentials and caller-selected prompt metadata. Handler bodies, browser calls, screenshot bytes, OCR text, registry calls, prompt IDs, prompt roles, prompt namespaces, prompt bodies, labels, and return strings remain redacted.
...
Source-defined tool handlers can also emit redacted visual-context credential-issuance bridge authority through `visual_context_credential_issuance_bridge`, `visual_context_capture`, `browser_automation`, `tainted_browser_automation_target`, `credential_issuance`, `tainted_credential_issuance_input`, and the corresponding `handler_*` fields when parsed MCP SDK or agent-framework tools capture authenticated browser screenshots, screen state, or OCR context from caller-selected targets and use that visual context as signing, grant, subject, scope, audience, tenant, or impersonation material to mint, sign, assume, impersonate, or issue agent credentials using runtime broker credentials. Handler bodies, browser calls, screenshot bytes, OCR text, credential broker calls, token templates, signing material, subjects, scopes, audiences, tenants, impersonation selectors, issued credentials, and return strings remain redacted.
...
Source-defined tool handlers can also emit redacted local-file prompt-registry bridge authority through `local_file_prompt_registry_bridge`, `tainted_filesystem_path`, `prompt_registry_write`, `tainted_prompt_registry_selector`, and the corresponding `handler_*` fields when parsed MCP SDK or agent-framework tools read caller-selected local files and publish the file material into prompt or instruction registries using runtime credentials and caller-selected prompt metadata. Handler bodies, filesystem reads, local paths, file contents, registry calls, prompt IDs, prompt roles, prompt bodies, prompt namespaces, and return strings remain redacted.
...
Source-defined tool handlers can also emit redacted RAG retrieval authorization-grant bridge authority through `rag_retrieval_authorization_grant_bridge`, `rag_retrieval`, `tainted_rag_retrieval_query`, `authorization_policy_write`, `tainted_authorization_grant_input`, `authorization_broad_grant`, and the corresponding `handler_*` fields when parsed MCP SDK or agent-framework tools retrieve caller-selected vector or RAG context and use the returned chunks to write broad, wildcard, approval-free, or long-lived authorization, permission, entitlement, or tool-grant policy using runtime credentials. Handler bodies, retriever calls, query text, namespaces, filters, retrieved chunks, grant calls, tool names, roles, scopes, subjects, resources, tenant selectors, grant reasons, and return strings remain redacted.
...
Source-defined tool handlers can also emit redacted RAG retrieval safety-policy bridge authority through `rag_retrieval_safety_policy_bridge`, `rag_retrieval`, `tainted_rag_retrieval_query`, `safety_policy_write`, `tainted_safety_policy_payload`, `tainted_safety_policy_selector`, `safety_policy_weakening`, and the corresponding `handler_*` fields when parsed MCP SDK or agent-framework tools retrieve caller-selected vector or RAG context and use returned chunks to update guardrail, approval, moderation, or safety policy with disabled, monitor-only, fail-open, default-allow, or approval-off posture. Handler bodies, retriever calls, query text, namespaces, filters, retrieved chunks, retrieved-context policy variables, policy update calls, target control selectors, selected modes, policy payloads, and return strings remain redacted.
...
Source-defined tool handlers can also emit redacted clipboard credential-issuance bridge authority through `clipboard_credential_issuance_bridge`, `clipboard_read`, `credential_issuance`, `tainted_credential_issuance_input`, and the corresponding `handler_*` fields when parsed MCP SDK or agent-framework tools read desktop or browser clipboard material and use copied text as grant material, token templates, subjects, scopes, roles, audiences, signing inputs, or impersonation selectors for minting, signing, assuming, impersonating, or issuing agent credentials using runtime broker credentials. Handler bodies, clipboard helper calls, copied text, derived clipboard variables, credential broker calls, subjects, audiences, grant material, issued credentials, and return strings remain redacted.
...
Source-defined tool handlers can also emit redacted RAG retrieval credential-issuance bridge authority through `rag_retrieval_credential_issuance_bridge`, `rag_retrieval`, `tainted_rag_retrieval_query`, `credential_issuance`, `tainted_credential_issuance_input`, and the corresponding `handler_*` fields when parsed MCP SDK or agent-framework tools retrieve caller-selected vector or RAG context and use returned context as signing, broker, or grant material to mint, sign, assume, impersonate, or issue agent credentials using runtime broker credentials. Handler bodies, retriever calls, query text, namespaces, filters, retrieved chunks, signing material, credential broker calls, subjects, scopes, roles, audiences, impersonation selectors, issued credentials, and return strings remain redacted.
...
Source-defined tool handlers can also emit redacted visual-context feedback bridge authority through `visual_context_feedback_bridge`, `visual_context_capture`, `feedback_pipeline_write`, `feedback_auto_promotion`, `tainted_feedback_routing`, and the corresponding `handler_*` fields when parsed MCP SDK or agent-framework tools capture authenticated browser or screen visual context from caller-selected targets and promote screenshot or OCR context into feedback, RLHF, eval, reward-model, or model-improvement pipelines using runtime credentials and caller-selected routing. Handler bodies, browser page calls, screenshots, OCR text, feedback pipeline calls, dataset IDs, eval-set IDs, promotion targets, reviewer notes, feedback records, and return strings remain redacted.
...
- baseline rule-pack fingerprint when the baseline is an AgentCSP manifest that includes `metadata.rule_pack.fingerprint`
...
If the baseline file is outside the scanned root, `baseline_path` is emitted as `<external-baseline>` so scan artifacts and baseline read errors do not expose external local or CI filesystem layout. `baseline_fingerprint` and `baseline_rule_pack_fingerprint` copy only prior digest metadata, not prior raw findings, evidence, root paths, or timestamps.
