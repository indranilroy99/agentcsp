...
- handler metadata for external network writes, external service SDK writes, model-provider SDK calls, privileged prompt-role composition, credentialed network reads, network response capture into model-visible output, memory/vector/RAG/state persistence, agent control-plane file writes, credential issuance or identity impersonation, nested tool invocation, browser automation, secret-manager and vault reads, environment-backed secret access, secret-to-output materialization, database query/write execution, shell execution, caller-controlled command arguments passed to shell execution, dynamic in-process code execution, caller-controlled code arguments passed to dynamic execution, unsafe deserialization, arbitrary local file disclosure, filesystem reads/writes/deletes, signal counts, authority categories, and env key names without emitting handler bodies, URLs, headers, SQL strings, memory-write snippets, agent-config write snippets, credential-broker calls, nested-tool dispatcher calls, browser page calls, secret-manager read calls, external service SDK calls, model-provider SDK calls, privileged prompt snippets, shell/subprocess snippets, returned network-response snippets, returned secret-bearing text, dynamic-code snippets, deserialization snippets, file-read snippets, source snippets, or secret values
...
- regression coverage proving raw handler syntax such as network calls, network response variables, memory-store writes, agent control-plane write calls and target paths, credential-broker calls, identity-broker calls, nested-tool dispatcher calls, browser page calls, secret-manager read calls, external service SDK calls, model-provider SDK calls, privileged prompt-role snippets, database driver calls, SQL strings, returned network-response text, returned secret-bearing text, filesystem read/delete calls, shell/subprocess calls, command argument snippets, dynamic-code evaluation calls, dynamic code argument snippets, unsafe deserialization calls, `os.getenv`, credentialed fetch calls, and bearer-header construction stays redacted
