...
- handler metadata for external network writes, caller-selected URLs, endpoints, webhooks, or destinations passed into HTTP client calls, external service SDK writes, caller-selected external-service recipients or channels passed into SDK writes, model-provider SDK calls, caller-selected model, deployment, provider, endpoint, or base URL routing passed into model-provider SDK calls, caller-selected URL responses persisted into durable memory/RAG/state stores, caller-selected URL responses published through external-service SDKs, raw nested-tool observations returned into model-visible output, raw nested-tool observations persisted into memory/RAG/state stores, raw nested-tool observations published through external-service SDKs with runtime credentials, secret-manager outputs published through external-service SDKs with runtime credentials, secret-manager outputs exported into training, fine-tuning, eval, or model-improvement datasets with runtime credentials, secret-manager outputs promoted through feedback, RLHF, eval, reward-model, or model-improvement pipelines with runtime credentials, embedding-provider calls with caller/customer text persisted into vector memory, telemetry/tracing export calls with caller/customer/prompt/tool-output payloads, prompt-cache writes with caller-controlled cache keys and prompt/customer/tool-output values, AI training or fine-tuning dataset exports with caller-controlled prompt/customer/tool-output payloads, feedback/RLHF pipeline writes with caller/customer feedback and raw agent context auto-promoted to caller-selected training, eval, reward-model, or model-update paths, guardrail/approval/moderation/safety-policy writes with caller-controlled policy payloads, caller-selected controls or modes, and explicit weakening posture, authorization/permission/entitlement/tool-grant writes with caller-controlled tools, scopes, roles, resources, tenants, subjects, or reasons and broad or approval-free grant posture, RAG/vector retrieval with caller-selected query, namespace, tenant, collection, or filter inputs returned as raw model-visible context, async task/agent queue enqueue with caller/customer payloads and caller-selected queue, topic, route, worker, or tenant routing, prompt/instruction registry writes with caller/customer prompt content and caller-selected prompt IDs, roles, namespaces, versions, or environments, authenticated browser or screen visual-context capture embedded by external providers and persisted into vector memory with runtime credentials and caller-selected namespaces, authenticated browser or screen visual-context capture promoted into feedback, RLHF, eval, reward-model, or model-improvement pipelines with runtime credentials and caller-selected routing, authenticated browser or screen visual-context capture returned to model-visible output from caller-selected targets, remote-agent/A2A delegation with caller-selected targets and forwarded caller/customer context, privileged prompt-role composition, credentialed network reads, network response capture into model-visible output, network response persistence into memory, network response publication to external services, memory/vector/RAG/state persistence, caller-selected memory namespaces or scopes passed into durable memory writes, agent control-plane file writes, credential issuance or identity impersonation, caller-selected subjects, scopes, roles, audiences, tenants, or accounts passed into credential broker APIs, nested tool invocation, browser automation, caller-selected URLs, selectors, form inputs, or customer payloads passed into browser page/driver calls, secret-manager and vault reads, caller-controlled secret path arguments passed to vault APIs, environment-backed secret access, secret-to-output materialization, database query/write execution, caller-controlled SQL/query arguments passed into database APIs, shell execution, caller-controlled command arguments passed to shell execution, caller-controlled path arguments passed into filesystem APIs, dynamic in-process code execution, caller-controlled code arguments passed to dynamic execution, unsafe deserialization, caller-controlled serialized payloads passed to unsafe loaders, arbitrary local file disclosure, filesystem reads/writes/deletes, signal counts, authority categories, and env key names without emitting handler bodies, URLs, headers, SQL strings, caller-controlled network destination snippets, caller-controlled credential grant snippets, caller-controlled SQL/query snippets, caller-controlled model-routing snippets, returned network-response memory snippets, returned network-response external-service snippets, raw tool-observation return snippets, raw tool-observation memory snippets, raw tool-observation external-post snippets, secret-manager external-service bridge snippets, secret-manager training dataset bridge snippets, secret-manager feedback bridge snippets, embedding-provider snippets, retriever calls, retrieval query/filter/namespace values, retrieved chunks, document IDs, queue calls, queue names, topics, routes, queued payloads, requested action labels, registry calls, prompt IDs, prompt names, roles, namespaces, versions, prompt bodies, customer context, telemetry calls, trace payloads, prompt-cache calls, cache keys, cache values, training dataset calls, dataset IDs, training records, feedback pipeline calls, eval-set IDs, promotion targets, feedback records, reviewer notes, safety-policy calls, policy payloads, control IDs, selected policy modes, authorization grant calls, granted tool names, scopes, roles, subjects, resources, tenant selectors, screenshot bytes, OCR text, visual-context feedback records, reviewer notes, delegated-agent calls, forwarded context snippets, memory-write snippets, caller-selected memory scope snippets, agent-config write snippets, credential-broker calls, nested-tool dispatcher calls, browser page calls, caller-selected browser target snippets, screenshot calls, visual-capture output snippets, secret-manager read calls, external service SDK calls, caller-selected external-service recipient snippets, model-provider SDK calls, privileged prompt snippets, shell/subprocess snippets, returned network-response snippets, returned retrieved context, returned secret-bearing text, secret external-post return strings, secret training-data return strings, secret feedback-promotion return strings, dynamic-code snippets, deserialization snippets, file-read snippets, source snippets, or secret values
- handler metadata for raw nested-tool observations published through Slack, email, chat, issue-tracker, or SaaS SDKs with runtime credentials, without emitting SDK calls, channel IDs, serialized tool observations, external-post snippets, return strings, or secret values
- handler metadata for raw nested-tool observations exported into public or shareable artifact/output storage with runtime credentials, without emitting nested tool calls, storage calls, object keys, bucket names, public URLs, raw observations, artifact contents, return strings, or secret values
- handler metadata for raw nested-tool observations exported into training, fine-tuning, eval, or model-improvement datasets with runtime credentials, without emitting nested tool calls, dataset calls, dataset IDs, source labels, raw observations, training records, return strings, or secret values
- handler metadata for raw nested-tool observations enqueued into asynchronous background-agent jobs with runtime credentials, without emitting nested tool calls, queue calls, queue names, routes, replay flags, queued payloads, raw observations, return strings, or secret values
- handler metadata for raw nested-tool observations published into prompt or instruction registries with runtime credentials and caller-selected prompt metadata, without emitting nested tool calls, registry calls, prompt IDs, prompt roles, raw observations, prompt bodies, return strings, or secret values
- handler metadata for caller-selected local file paths whose file material is exported into training, fine-tuning, eval, or model-improvement datasets with runtime training credentials and caller-selected dataset routing, without emitting file-read calls, local paths, file contents, dataset calls, dataset IDs, split names, source labels, training records, return strings, or secret values
- handler metadata for caller-selected local file paths whose file material is written into customer or operational database records with runtime database credentials, without emitting file-read calls, local paths, file contents, database calls, SQL text, record IDs, reviewer notes, payloads, return strings, or secret values
- handler metadata for caller-selected secret-manager paths whose returned secret material is published through Slack, email, chat, issue-tracker, or SaaS SDKs with runtime credentials, without emitting vault calls, secret values, SDK calls, recipient snippets, posted payloads, external-post snippets, return strings, or secret values
- handler metadata for caller-selected secret-manager paths whose returned secret material is forwarded into model-provider prompts with runtime credentials, without emitting vault calls, secret values, model SDK calls, prompt snippets, model responses, return strings, or secret values
- handler metadata for caller-selected secret-manager paths whose returned secret material is exported into AI training, fine-tuning, eval, or model-improvement datasets with runtime credentials, without emitting vault calls, secret values, dataset calls, dataset IDs, training records, return strings, or secret values
- handler metadata for caller-selected secret-manager paths whose returned secret material is promoted through feedback, RLHF, eval, reward-model, or model-improvement pipelines with runtime credentials, without emitting vault calls, secret values, feedback pipeline calls, dataset IDs, eval-set IDs, promotion targets, records, return strings, or secret values
- handler metadata for caller-selected secret-manager paths whose returned secret material is exported into AI telemetry, tracing, logging, or observability systems with runtime credentials, without emitting vault calls, secret values, telemetry calls, trace IDs, trace payloads, return strings, or secret values
- handler metadata for caller-selected secret-manager paths whose returned secret material is written into prompt, LLM, response, or semantic caches with runtime credentials and caller-controlled cache keys, without emitting vault calls, secret values, cache calls, cache keys, cache values, TTLs, return strings, or secret values
- handler metadata for caller-selected secret-manager paths whose returned secret material is used as authorization, permission, entitlement, or tool-grant material with runtime credentials and broad or approval-free posture, without emitting vault calls, secret values, grant calls, granted tool names, roles, scopes, subjects, resources, tenant selectors, grant reasons, return strings, or secret values
...
- rule `AGENTCSP-TOOL-141` for source-defined tools that read caller-selected local files and persist file material into prompt, LLM, response, or semantic caches under runtime cache credentials and caller-selected cache routing
...
- rule `AGENTCSP-TOOL-052` for source-defined tools that run caller-selected RAG/vector retrieval using runtime credentials and return raw retrieved chunks into model-visible output
- rule `AGENTCSP-TOOL-053` for source-defined tools that enqueue caller/customer payloads into caller-selected background agent queues, topics, routes, workers, or tenants using runtime credentials
- rule `AGENTCSP-TOOL-054` for source-defined tools that publish caller/customer prompt content into caller-selected prompt or instruction registry entries using runtime credentials
- source-defined local-file prompt-registry bridge metadata for parsed MCP SDK and agent-framework tools that read caller-selected local files and publish the file material into prompt or instruction registries with runtime credentials and caller-selected prompt metadata
- scanner and rule regression coverage proving local-file prompt-registry bridge findings fire for MCP SDK and LangChain fixtures without leaking filesystem reads, registry calls, local file variables, descriptions, or return strings
- rule `AGENTCSP-TOOL-171` for source-defined tools that publish caller-selected local file material into caller-selected prompt or instruction registry entries using runtime credentials
- source-defined local-file authorization-grant bridge metadata for parsed MCP SDK and agent-framework tools that read caller-selected local files and use the file material to write broad or approval-free authorization grants with runtime credentials
- scanner and rule regression coverage proving local-file authorization-grant bridge findings fire for MCP SDK and LangChain fixtures without leaking filesystem reads, grant calls, local file variables, descriptions, or return strings
- rule `AGENTCSP-TOOL-172` for source-defined tools that use caller-selected local file material to write broad or approval-free authorization, permission, entitlement, or tool-grant policy
- source-defined network-response authorization-grant bridge metadata for parsed MCP SDK and agent-framework tools that fetch caller-selected URLs and use returned response material to write broad or approval-free authorization grants with runtime credentials
- scanner and rule regression coverage proving network-response authorization-grant bridge findings fire for MCP SDK and LangChain fixtures without leaking network calls, response-derived grant variables, grant calls, descriptions, or return strings
- rule `AGENTCSP-TOOL-173` for source-defined tools that use caller-selected network response material to write broad or approval-free authorization, permission, entitlement, or tool-grant policy
- source-defined network-response safety-policy bridge metadata for parsed MCP SDK and agent-framework tools that fetch caller-selected URLs and use returned response material to weaken guardrail, approval, moderation, or safety policy with runtime credentials
- scanner and rule regression coverage proving network-response safety-policy bridge findings fire for MCP SDK and LangChain fixtures without leaking network calls, response-derived policy variables, policy update calls, descriptions, or return strings
- rule `AGENTCSP-TOOL-174` for source-defined tools that use caller-selected network response material to weaken guardrail, approval, moderation, or safety policy
- source-defined RAG retrieval safety-policy bridge metadata for parsed MCP SDK and agent-framework tools that retrieve caller-selected vector or RAG context and use returned chunks to weaken guardrail, approval, moderation, or safety policy with runtime credentials
- scanner and rule regression coverage proving RAG retrieval safety-policy bridge findings fire for MCP SDK and LangChain fixtures without leaking retriever calls, retrieved-context policy variables, policy update calls, descriptions, or return strings
- rule `AGENTCSP-TOOL-180` for source-defined tools that use retrieved RAG context to weaken guardrail, approval, moderation, or safety policy
- source-defined local-file safety-policy bridge metadata for parsed MCP SDK and agent-framework tools that read caller-selected local files and use file material to weaken guardrail, approval, moderation, or safety policy with runtime credentials
- scanner and rule regression coverage proving local-file safety-policy bridge findings fire for MCP SDK and LangChain fixtures without leaking filesystem reads, file-derived policy variables, policy update calls, descriptions, or return strings
- rule `AGENTCSP-TOOL-175` for source-defined tools that use caller-selected local file material to weaken guardrail, approval, moderation, or safety policy
- source-defined network-response credential-issuance bridge metadata for parsed MCP SDK and agent-framework tools that fetch caller-selected URLs and use returned response material as signing, broker, or grant material to issue credentials with runtime broker credentials
- scanner and rule regression coverage proving network-response credential-issuance bridge findings fire for MCP SDK and LangChain fixtures without leaking network calls, response-derived signing variables, credential broker calls, descriptions, or return strings
- rule `AGENTCSP-TOOL-176` for source-defined tools that use caller-selected network response material to mint, sign, assume, impersonate, or issue agent credentials
- source-defined clipboard authorization-grant bridge metadata for parsed MCP SDK and agent-framework tools that read desktop clipboard content and use copied text to write broad or approval-free authorization grants with runtime credentials
- scanner and rule regression coverage proving clipboard authorization-grant bridge findings fire for MCP SDK and LangChain fixtures without leaking clipboard reads, copied-text variables, grant calls, descriptions, or return strings
- rule `AGENTCSP-TOOL-177` for source-defined tools that use clipboard material to write broad or approval-free authorization, permission, entitlement, or tool-grant policy
- source-defined RAG retrieval authorization-grant bridge metadata for parsed MCP SDK and agent-framework tools that retrieve caller-selected vector or RAG context and use returned chunks to write broad or approval-free authorization grants with runtime credentials
- scanner and rule regression coverage proving RAG retrieval authorization-grant bridge findings fire for MCP SDK and LangChain fixtures without leaking retriever calls, retrieved-context variables, grant calls, descriptions, or return strings
- rule `AGENTCSP-TOOL-178` for source-defined tools that use retrieved RAG context to write broad or approval-free authorization, permission, entitlement, or tool-grant policy
- source-defined RAG retrieval credential-issuance bridge metadata for parsed MCP SDK and agent-framework tools that retrieve caller-selected vector or RAG context and use returned chunks as signing, broker, or grant material to issue credentials with runtime broker credentials
- scanner and rule regression coverage proving RAG retrieval credential-issuance bridge findings fire for MCP SDK and LangChain fixtures without leaking retriever calls, retrieved-context variables, credential broker calls, descriptions, or return strings
- rule `AGENTCSP-TOOL-179` for source-defined tools that use retrieved RAG context to mint, sign, assume, impersonate, or issue agent credentials
- source-defined RAG retrieval task-queue bridge metadata for parsed MCP SDK and agent-framework tools that retrieve caller-selected vector or RAG context and enqueue returned chunks into background-agent or task queues with runtime queue credentials and caller-selected routing
- scanner and rule regression coverage proving RAG retrieval task-queue bridge findings fire for MCP SDK and LangChain fixtures without leaking retriever calls, retrieved-context variables, queue calls, queued payloads, descriptions, requested action labels, or return strings
- rule `AGENTCSP-TOOL-181` for source-defined tools that queue retrieved RAG context into asynchronous background-agent execution
- source-defined RAG retrieval remote-agent delegation bridge metadata for parsed MCP SDK and agent-framework tools that retrieve caller-selected vector or RAG context and delegate returned chunks to remote agents or A2A peers with runtime delegation credentials and caller-selected targets
- scanner and rule regression coverage proving RAG retrieval remote-agent delegation findings fire for MCP SDK and LangChain fixtures without leaking retriever calls, retrieved-context variables, delegated-agent calls, forwarded context, descriptions, or return strings
- rule `AGENTCSP-TOOL-182` for source-defined tools that delegate retrieved RAG context across remote-agent trust boundaries
- source-defined RAG retrieval prompt-cache bridge metadata for parsed MCP SDK and agent-framework tools that retrieve caller-selected vector or RAG context and persist returned chunks into prompt, LLM, response, or semantic caches with runtime cache credentials and caller-selected cache scope
- scanner and rule regression coverage proving RAG retrieval prompt-cache bridge findings fire for MCP SDK and LangChain fixtures without leaking retriever calls, retrieved-context variables, prompt-cache calls, cache values, descriptions, or return strings
- rule `AGENTCSP-TOOL-183` for source-defined tools that persist retrieved RAG context into replayable prompt or semantic caches
- source-defined RAG retrieval prompt-registry bridge metadata for parsed MCP SDK and agent-framework tools that retrieve caller-selected vector or RAG context and publish returned chunks into reusable prompt or instruction registry entries with runtime registry credentials and caller-selected prompt metadata
- scanner and rule regression coverage proving RAG retrieval prompt-registry findings fire for MCP SDK and LangChain fixtures without leaking retriever calls, retrieved-context variables, prompt-registry calls, prompt bodies, descriptions, or return strings
- rule `AGENTCSP-TOOL-184` for source-defined tools that persist retrieved RAG context into reusable prompt registry entries
- source-defined RAG retrieval training-dataset bridge metadata for parsed MCP SDK and agent-framework tools that retrieve caller-selected vector or RAG context and export returned chunks into training, fine-tuning, eval, or model-improvement datasets with runtime dataset credentials and caller-selected dataset routing
- scanner and rule regression coverage proving RAG retrieval training-dataset findings fire for MCP SDK and LangChain fixtures without leaking retriever calls, retrieved-context variables, training dataset calls, training records, descriptions, or return strings
- rule `AGENTCSP-TOOL-185` for source-defined tools that persist retrieved RAG context into training, eval, fine-tuning, or model-improvement datasets
- source-defined RAG retrieval artifact bridge metadata for parsed MCP SDK and agent-framework tools that retrieve caller-selected vector or RAG context and export returned chunks into public or shareable artifacts with runtime storage credentials and caller-selected artifact routing
- scanner and rule regression coverage proving RAG retrieval artifact findings fire for MCP SDK and LangChain fixtures without leaking retriever calls, retrieved-context variables, artifact calls, artifact payloads, descriptions, or return strings
- rule `AGENTCSP-TOOL-186` for source-defined tools that publish retrieved RAG context into public or shareable artifacts
- rule `AGENTCSP-TOOL-055` for source-defined tools that record caller/customer feedback and raw agent context into feedback, RLHF, eval, or model-improvement pipelines with caller-selected promotion routing using runtime credentials
- rule `AGENTCSP-TOOL-056` for source-defined tools that update guardrail, approval, moderation, or safety policy using runtime credentials, caller-controlled policy content, caller-selected controls or modes, and explicit weakening such as default-allow, disabled enforcement, or approval-off posture
- rule `AGENTCSP-TOOL-057` for source-defined tools that write authorization, permission, entitlement, or tool-grant policy using runtime credentials, caller-controlled tool/scope/role/resource/tenant/subject inputs, and broad or approval-free grant posture
- rule `AGENTCSP-TOOL-058` for source-defined tools that invoke nested tools and forward raw tool observations into model-provider prompts using runtime credentials
- rule `AGENTCSP-TOOL-059` for source-defined tools that invoke nested tools and persist raw tool observations into durable memory, RAG, or state stores using runtime credentials
- rule `AGENTCSP-TOOL-060` for source-defined tools that invoke nested tools and return raw tool observations into model-visible output
- rule `AGENTCSP-TOOL-061` for source-defined tools that invoke nested tools and publish raw tool observations through external-service SDKs using runtime credentials
- rule `AGENTCSP-TOOL-062` for source-defined tools that read caller-selected secret-manager paths and publish returned secret material through external-service SDKs using runtime credentials
- rule `AGENTCSP-TOOL-063` for source-defined tools that read caller-selected secret-manager paths and forward returned secret material into model-provider prompts using runtime credentials
- rule `AGENTCSP-TOOL-064` for source-defined tools that read caller-selected secret-manager paths and persist returned secret material into durable memory, RAG, vector, or state stores using runtime credentials
- rule `AGENTCSP-TOOL-091` for source-defined tools that read caller-selected secret-manager values and write raw secrets into customer or operational database records using runtime authority
- rule `AGENTCSP-TOOL-065` for source-defined tools that read caller-selected secret-manager paths and export returned secret material into AI training, fine-tuning, eval, or model-improvement datasets using runtime credentials
- rule `AGENTCSP-TOOL-066` for source-defined tools that read caller-selected secret-manager paths and promote returned secret material through feedback, RLHF, eval, reward-model, or model-improvement pipelines using runtime credentials
- rule `AGENTCSP-TOOL-067` for source-defined tools that read caller-selected secret-manager paths and export returned secret material into public or shareable artifact/output storage using runtime credentials
- rule `AGENTCSP-TOOL-068` for source-defined tools that read caller-selected secret-manager paths and export returned secret material into AI telemetry, tracing, logging, or observability systems using runtime credentials
- rule `AGENTCSP-TOOL-069` for source-defined tools that read caller-selected secret-manager paths and write returned secret material into prompt, LLM, response, or semantic caches using runtime credentials and caller-controlled cache keys
- rule `AGENTCSP-TOOL-070` for source-defined tools that read caller-selected secret-manager paths and publish returned secret material into prompt or instruction registries using runtime credentials and caller-selected prompt metadata
- rule `AGENTCSP-TOOL-071` for source-defined tools that invoke nested tools and publish raw tool observations into prompt or instruction registries using runtime credentials and caller-selected prompt metadata
- rule `AGENTCSP-TOOL-072` for source-defined tools that read caller-selected secret-manager paths, send returned secret material to embedding providers with runtime credentials, and persist the resulting vectors into memory, RAG, vector, or state stores
- rule `AGENTCSP-TOOL-073` for source-defined tools that read caller-selected secret-manager paths and use returned secret material to write broad, wildcard, approval-free, or long-lived authorization, permission, entitlement, or tool-grant policy using runtime credentials
- rule `AGENTCSP-TOOL-074` for source-defined tools that read caller-selected secret-manager paths and use returned secret material to mint, sign, assume, impersonate, or issue agent credentials using runtime broker credentials
- rule `AGENTCSP-TOOL-075` for OpenAPI tool imports that send prompt-like or freeform request content through authenticated external writes without approval
- rule `AGENTCSP-TOOL-076` for source-defined tools that invoke nested tools and use raw tool observations to write broad, wildcard, approval-free, or long-lived authorization, permission, entitlement, or tool-grant policy using runtime credentials
- rule `AGENTCSP-TOOL-077` for source-defined tools that invoke nested tools and use raw tool observations to mint, sign, assume, impersonate, or issue agent credentials using runtime broker credentials
- rule `AGENTCSP-TOOL-078` for source-defined tools that invoke nested tools and write raw tool observations into customer or operational databases using database authority
- rule `AGENTCSP-TOOL-079` for source-defined tools that invoke nested tools and export raw tool observations into AI telemetry, tracing, logging, or observability systems using runtime credentials
- rule `AGENTCSP-TOOL-080` for source-defined tools that invoke nested tools and export raw tool observations into public or shareable artifact/output storage using runtime credentials
- rule `AGENTCSP-TOOL-081` for source-defined tools that invoke nested tools and enqueue raw tool observations into asynchronous background-agent jobs using runtime credentials
- rule `AGENTCSP-TOOL-082` for source-defined tools that invoke nested tools and export raw tool observations into training, fine-tuning, eval, or model-improvement datasets using runtime credentials
- rule `AGENTCSP-TOOL-083` for source-defined tools that invoke nested tools and delegate raw tool observations to remote agents or A2A peers using runtime credentials
- rule `AGENTCSP-TOOL-084` for source-defined tools that read caller-selected secret-manager values and enqueue raw secrets into background-agent/task queues using runtime credentials
- rule `AGENTCSP-TOOL-085` for source-defined tools that read caller-selected secret-manager values and delegate raw secrets to remote agents or A2A peers using runtime credentials
- rule `AGENTCSP-TOOL-086` for source-defined tools that read caller-selected secret-manager values and inject raw secrets into authenticated browser automation with caller-selected targets or selectors
- rule `AGENTCSP-TOOL-087` for source-defined tools that invoke nested tools and inject raw tool observations into authenticated browser automation with caller-selected targets or selectors
- rule `AGENTCSP-TOOL-088` for source-defined tools that invoke nested tools and use raw tool observations to update or weaken guardrail, approval, moderation, or safety policy with runtime credentials
- rule `AGENTCSP-TOOL-089` for source-defined tools that read caller-selected secret-manager values and use raw secrets to update or weaken guardrail, approval, moderation, or safety policy with runtime credentials
- rule `AGENTCSP-TOOL-090` for source-defined tools that invoke nested tools, send raw tool observations into embedding providers, and persist the resulting vectors into replayable memory, RAG, vector, or state stores using runtime credentials
- rule `AGENTCSP-TOOL-092` for source-defined tools that invoke nested tools and write raw tool observations into prompt, LLM, response, or semantic caches using runtime credentials and caller-controlled cache keys
- rule `AGENTCSP-TOOL-093` for source-defined tools that retrieve caller-selected RAG or vector content and persist raw retrieved chunks into durable memory, RAG, or state stores using runtime credentials
- rule `AGENTCSP-TOOL-094` for source-defined tools that retrieve caller-selected RAG or vector content and forward raw retrieved chunks into model-provider prompts using runtime credentials
- rule `AGENTCSP-TOOL-095` for source-defined tools that retrieve caller-selected RAG or vector content and publish raw retrieved chunks through external-service SDKs using runtime credentials and caller-selected recipients
- rule `AGENTCSP-TOOL-096` for source-defined tools that retrieve caller-selected RAG or vector content and inject raw retrieved chunks into authenticated browser automation with runtime credentials and caller-selected targets or selectors
- rule `AGENTCSP-TOOL-097` for source-defined tools that capture authenticated browser or screen visual context from caller-selected targets and forward screenshots or OCR context into model-provider prompts using runtime credentials
- rule `AGENTCSP-TOOL-098` for source-defined tools that capture authenticated browser or screen visual context from caller-selected targets and publish screenshots or OCR context through external-service SDKs using runtime credentials and caller-selected recipients
- rule `AGENTCSP-TOOL-099` for source-defined tools that capture authenticated browser or screen visual context from caller-selected targets and persist screenshots or OCR context into durable memory, RAG, vector, or state stores using runtime credentials and caller-selected memory scopes
- rule `AGENTCSP-TOOL-100` for source-defined tools that capture authenticated browser or screen visual context from caller-selected targets and export screenshots or OCR context into public or shareable artifact storage using runtime credentials and caller-selected artifact destinations
- rule `AGENTCSP-TOOL-101` for source-defined tools that capture authenticated browser or screen visual context from caller-selected targets and export screenshots or OCR context into training, fine-tuning, eval, feedback, or model-improvement datasets using runtime credentials and caller-selected dataset routing
- rule `AGENTCSP-TOOL-102` for source-defined tools that capture authenticated browser or screen visual context from caller-selected targets and export screenshots or OCR context into AI telemetry, tracing, logging, or observability systems using runtime credentials and caller-selected trace metadata
- rule `AGENTCSP-TOOL-103` for source-defined tools that capture authenticated browser or screen visual context from caller-selected targets and write screenshots or OCR context into prompt, LLM, response, or semantic caches using runtime credentials and caller-controlled cache keys
- rule `AGENTCSP-TOOL-104` for source-defined tools that capture authenticated browser or screen visual context from caller-selected targets and enqueue screenshots or OCR context into asynchronous background-agent jobs using runtime credentials and caller-selected queue routing
- rule `AGENTCSP-TOOL-105` for source-defined tools that capture authenticated browser or screen visual context from caller-selected targets and delegate screenshots or OCR context to remote agents or A2A peers using runtime credentials and caller-selected agent targets
- rule `AGENTCSP-TOOL-187` for source-defined tools that capture authenticated browser or screen visual context from caller-selected targets and publish screenshots or OCR context into reusable prompt or instruction registries using runtime credentials and caller-selected prompt metadata
- rule `AGENTCSP-TOOL-188` for source-defined tools that capture authenticated browser or screen visual context from caller-selected targets and write screenshots or OCR context into customer or operational databases using runtime credentials and caller-selected records
- rule `AGENTCSP-TOOL-189` for source-defined tools that capture authenticated browser or screen visual context from caller-selected targets and use screenshot or OCR context to write broad authorization grants using runtime credentials and caller-selected grant routing
- rule `AGENTCSP-TOOL-190` for source-defined tools that capture authenticated browser or screen visual context from caller-selected targets and use screenshot or OCR context to weaken guardrail, approval, moderation, or safety policy using runtime policy credentials and caller-selected policy/control routing
- rule `AGENTCSP-TOOL-191` for source-defined tools that capture authenticated browser or screen visual context from caller-selected targets and use screenshot or OCR context to mint, sign, assume, impersonate, or issue agent credentials using runtime broker credentials and caller-selected credential routing
- rule `AGENTCSP-TOOL-192` for source-defined tools that capture authenticated browser or screen visual context from caller-selected targets and promote screenshot or OCR context into feedback, RLHF, eval, reward-model, or model-improvement pipelines using runtime credentials and caller-selected routing
- rule `AGENTCSP-TOOL-193` for source-defined tools that capture authenticated browser or screen visual context from caller-selected targets, send screenshot or OCR context to embedding providers, and persist vectors into memory or vector stores using runtime credentials and caller-selected namespaces
- rule `AGENTCSP-TOOL-194` for source-defined tools that capture authenticated browser or screen visual context from caller-selected targets and pass screenshot or OCR-derived context into shell or subprocess execution under runtime authority
- scanner and rule regression coverage proving visual-context shell-execution bridge findings fire for MCP SDK and LangChain fixtures without leaking browser calls, screenshot bytes, shell calls, command variables, descriptions, or return strings
- rule `AGENTCSP-TOOL-195` for source-defined tools that capture authenticated browser or screen visual context from caller-selected targets and pass screenshot or OCR-derived context into dynamic JavaScript or Python code execution under runtime authority
- scanner and rule regression coverage proving visual-context dynamic-code bridge findings fire for MCP SDK and LangChain fixtures without leaking browser calls, screenshot bytes, eval or exec calls, generated-code variables, descriptions, or return strings
- rule `AGENTCSP-TOOL-106` for source-defined tools that read caller-selected local file paths and upload file material through authenticated browser automation to caller-selected web targets using runtime credentials
- rule `AGENTCSP-TOOL-107` for source-defined tools that read browser or desktop clipboard material and publish it through external-service SDKs using runtime credentials and caller-selected recipients
- rule `AGENTCSP-TOOL-108` for source-defined tools that call model providers and execute model-generated code through dynamic JavaScript or Python execution sinks under runtime authority
- rule `AGENTCSP-TOOL-109` for source-defined tools that call model providers and use model-selected network destinations for credential-backed fetch or request paths
- rule `AGENTCSP-TOOL-110` for source-defined tools that call model providers and run model-generated shell or subprocess commands under runtime authority
- rule `AGENTCSP-TOOL-111` for source-defined tools that invoke nested tools and run raw tool observations through shell or subprocess execution sinks under runtime authority
- rule `AGENTCSP-TOOL-112` for source-defined tools that invoke nested tools and execute raw tool observations through dynamic JavaScript or Python code execution sinks under runtime authority
- rule `AGENTCSP-TOOL-113` for source-defined tools that invoke nested tools and use raw tool observations as credentialed network destinations under runtime authority
- rule `AGENTCSP-TOOL-114` for source-defined tools that call model providers and use model responses as authenticated browser navigation or action targets under runtime authority
- rule `AGENTCSP-TOOL-115` for source-defined tools that call model providers and write model responses into customer or operational databases under runtime authority
- rule `AGENTCSP-TOOL-116` for source-defined tools that call model providers and use model responses to write broad or approval-free authorization grants under runtime authority
- rule `AGENTCSP-TOOL-117` for source-defined tools that call model providers and use model responses as grant material or selectors for issuing agent credentials under runtime broker authority
- rule `AGENTCSP-TOOL-118` for source-defined tools that call model providers and enqueue model responses into asynchronous background-agent/task queues under runtime queue authority
- rule `AGENTCSP-TOOL-119` for source-defined tools that call model providers and delegate model responses to remote agents or A2A peers under runtime delegation authority
- rule `AGENTCSP-TOOL-120` for source-defined tools that call model providers and publish model responses into prompt or instruction registries under runtime registry authority
- rule `AGENTCSP-TOOL-121` for source-defined tools that call model providers and use model responses to weaken guardrail, approval, moderation, or safety policy under runtime policy authority
- rule `AGENTCSP-TOOL-122` for source-defined tools that call model providers and publish model responses through external-service SDKs under runtime service authority and caller-selected recipient routing
- rule `AGENTCSP-TOOL-123` for source-defined tools that call model providers and persist model responses into durable memory, RAG, vector, or state stores under runtime memory-write authority and caller-selected memory scope
- rule `AGENTCSP-TOOL-124` for source-defined tools that call model providers and export model responses into training, fine-tuning, eval, or model-improvement datasets under runtime dataset-write authority and caller-selected dataset routing
- rule `AGENTCSP-TOOL-125` for source-defined tools that call model providers and export model responses into public or shareable artifact storage under runtime artifact-write authority and caller-selected object routing
- rule `AGENTCSP-TOOL-126` for source-defined tools that call model providers and export model responses into telemetry, tracing, logging, or observability systems under runtime telemetry authority and caller-selected trace routing
- rule `AGENTCSP-TOOL-127` for source-defined tools that call model providers and persist generated responses into prompt, LLM, response, or semantic caches under runtime cache authority and caller-selected cache routing
- rule `AGENTCSP-TOOL-128` for source-defined tools that call model providers and promote generated responses into feedback, RLHF, eval, reward-model, or model-improvement pipelines under runtime feedback authority and caller-selected promotion routing
- rule `AGENTCSP-TOOL-129` for source-defined tools that invoke nested tools and promote raw tool observations into feedback, RLHF, eval, reward-model, or model-improvement pipelines under runtime feedback authority and caller-selected promotion routing
- rule `AGENTCSP-TOOL-130` for source-defined tools that fetch caller-selected URLs and persist raw network responses into durable memory, RAG, vector, or state stores under runtime memory authority and caller-selected memory scope
- rule `AGENTCSP-TOOL-131` for source-defined tools that fetch caller-selected URLs and publish raw network responses through external-service SDKs under runtime service authority and caller-selected recipient routing
- rule `AGENTCSP-TOOL-132` for source-defined tools that fetch caller-selected URLs and persist raw network responses into prompt, LLM, response, or semantic caches under runtime cache authority and caller-selected cache routing
- rule `AGENTCSP-TOOL-133` for source-defined tools that fetch caller-selected URLs and export raw network responses into training, fine-tuning, eval, or model-improvement datasets under runtime dataset authority and caller-selected dataset routing
- rule `AGENTCSP-TOOL-134` for source-defined tools that fetch caller-selected URLs and export raw network responses into telemetry, tracing, logging, or observability systems under runtime telemetry authority and caller-selected trace routing
- rule `AGENTCSP-TOOL-135` for source-defined tools that fetch caller-selected URLs and export raw network responses into public or shareable run artifacts under runtime artifact authority and caller-selected artifact routing
- rule `AGENTCSP-TOOL-136` for source-defined tools that fetch caller-selected URLs and queue raw network responses into background-agent or task queues under runtime queue authority and caller-selected route selection
- rule `AGENTCSP-TOOL-137` for source-defined tools that fetch caller-selected URLs and delegate raw network responses to remote agents or A2A peers under runtime delegation authority and caller-selected target selection
- rule `AGENTCSP-TOOL-138` for source-defined tools that fetch caller-selected URLs and inject raw network responses into authenticated browser automation under runtime browser authority and caller-selected target or selector selection
- rule `AGENTCSP-TOOL-139` for source-defined tools that fetch caller-selected URLs and publish raw network responses into prompt or instruction registries under runtime registry authority and caller-selected prompt metadata
- regression coverage proving network-response prompt-cache snippets, cache calls, cache keys, response variables, response text, and network-response prompt-cache return strings stay redacted
- regression coverage proving network-response prompt-registry snippets, registry calls, prompt IDs, prompt roles, namespaces, response variables, response text, and network-response prompt-registry return strings stay redacted
- regression coverage proving network-response training-dataset snippets, dataset calls, dataset IDs, split names, source labels, response variables, response text, and network-response training-data return strings stay redacted
- regression coverage proving network-response telemetry snippets, trace calls, trace IDs, project labels, response variables, response text, and network-response telemetry return strings stay redacted
- regression coverage proving network-response artifact snippets, artifact calls, object keys, bucket names, response variables, response text, public artifact routing, and network-response artifact return strings stay redacted
- regression coverage proving network-response task-queue snippets, queue calls, queue names, routes, job goals, response variables, response text, and network-response queue return strings stay redacted
- regression coverage proving network-response browser-automation snippets, browser page calls, browser targets, selectors, submitted response variables, response text, and network-response browser return strings stay redacted
...
- regression coverage proving raw handler syntax such as network calls, network response variables, network-response memory bridge snippets, network-response external-service snippets, network-response prompt-registry snippets, memory-store writes, telemetry/tracing SDK calls, trace payload variables, prompt-cache calls, cache keys, cache values, prompt-registry secret bridge calls, prompt IDs, prompt roles, secret prompt-registry variables, training dataset calls, dataset IDs, training records, feedback pipeline calls, eval-set IDs, promotion targets, feedback records, reviewer notes, feedback promotion return strings, safety-policy calls, policy payloads, policy update return strings, authorization grant calls, grant payloads, authorization update return strings, secret authorization-grant bridge snippets, secret-derived grant variables, secret grant return strings, model-output credential-issuance bridge snippets, model-derived credential grant variables, model credential return strings, model-output task-queue bridge snippets, model-derived queued job variables, model queue return strings, model-output remote-agent delegation bridge snippets, model-derived delegated task variables, model delegation return strings, model-output prompt-registry bridge snippets, model-derived prompt-registry variables, model prompt-registry return strings, model-output external-service bridge snippets, model-derived external message variables, model external-post return strings, model-output memory bridge snippets, model-derived memory variables, model memory-persistence return strings, model-output training-dataset bridge snippets, model-derived training-record variables, model training-data return strings, model-output artifact bridge snippets, model-derived artifact variables, model artifact-export return strings, model-output telemetry bridge snippets, model-derived trace variables, model trace-export return strings, model-output prompt-cache bridge snippets, model-derived cache variables, model prompt-cache return strings, model-output feedback bridge snippets, model-derived feedback variables, model feedback-promotion return strings, secret credential-issuance bridge snippets, secret-derived signing key variables, secret credential return strings, tool-output credential-issuance snippets, tool-output grant-material variables, tool-output credential return strings, tool-output prompt-cache bridge snippets, tool-output prompt-cache return strings, tool-output database bridge snippets, tool-output database return strings, tool-output feedback bridge snippets, artifact export calls, public artifact URLs, object keys, bucket names, artifact return strings, retriever calls, retrieval query/filter/namespace values, retrieved chunks, retrieved-context prompt bridge snippets, retrieved-context model return strings, retrieved-context external-service bridge snippets, retrieved-context external-post return strings, retrieved-context memory bridge snippets, retrieved-context memory return strings, retrieved-context browser-automation bridge snippets, retrieved-context browser-submission return strings, document IDs, returned retrieved context, queue enqueue calls, queue names, topics, routes, queued payloads, requested action labels, queue return strings, prompt-registry calls, prompt IDs, prompt names, roles, namespaces, versions, prompt bodies, customer context, registry return strings, approval model calls, approval decision summaries, privileged action executor calls, action payloads, agent control-plane write calls and target paths, credential-broker calls, identity-broker calls, nested-tool dispatcher calls, raw tool-observation variables, tool-output embedding-vector bridge snippets, tool-output telemetry bridge snippets, tool-output artifact bridge snippets, tool-output training-dataset bridge snippets, tool-output task-queue bridge snippets, tool-output remote-agent delegation bridge snippets, tool-output browser-automation bridge snippets, tool-output return snippets, model prompt bridge snippets, tool-output prompt-registry bridge snippets, tool-output authorization-grant bridge snippets, secret-manager prompt bridge snippets, secret-manager memory bridge snippets, secret-manager database bridge snippets, secret-manager embedding-vector bridge snippets, secret-manager authorization-grant bridge snippets, secret-manager credential-issuance bridge snippets, secret-manager training dataset bridge snippets, secret-manager feedback bridge snippets, secret-manager artifact bridge snippets, secret-manager telemetry bridge snippets, secret-manager prompt-cache bridge snippets, secret-manager task-queue bridge snippets, secret-manager remote-agent delegation bridge snippets, secret-manager browser-automation bridge snippets, secret-manager prompt-registry bridge snippets, memory bridge snippets, external-service bridge snippets, secret-manager external-service bridge snippets, network-response memory return strings, network-response external-post return strings, network-response remote-agent delegation snippets, network-response remote-agent return strings, observation-memory return strings, observation vector-memory return strings, observation prompt-cache return strings, observation telemetry-export return strings, observation artifact-export return strings, observation training-data return strings, observation feedback-promotion return strings, observation queue-enqueue return strings, observation remote-agent delegation return strings, observation browser-submission return strings, observation external-post return strings, observation prompt-registry return strings, observation authorization-grant return strings, observation credential-issuance return strings, secret model-return strings, secret memory-persistence return strings, secret database-write return strings, secret vector-memory return strings, secret authorization-grant return strings, secret credential-issuance return strings, secret training-data return strings, secret feedback-promotion return strings, secret artifact-export return strings, secret telemetry-export return strings, secret prompt-cache return strings, secret queue-enqueue return strings, secret remote-agent delegation return strings, secret browser-submission return strings, secret prompt-registry return strings, secret external-post return strings, delegated-agent calls, forwarded context snippets, browser page calls, screenshot calls, visual-capture outputs, visual-context prompt bridge snippets, visual-context model return strings, visual-context external-service bridge snippets, visual-context external-post return strings, visual-context memory bridge snippets, visual-context memory return strings, visual-context embedding-vector bridge snippets, visual-context vector-memory return strings, secret-manager read calls, caller-controlled vault path snippets, external service SDK calls, model-provider SDK calls, privileged prompt-role snippets, database driver calls, SQL strings, returned network-response text, returned secret-bearing text, secret-record variables, filesystem read/delete calls, caller-controlled filesystem path snippets, shell/subprocess snippets, command argument snippets, dynamic-code evaluation calls, dynamic code argument snippets, unsafe deserialization calls, serialized payload transformation snippets, `os.getenv`, credentialed fetch calls, and bearer-header construction stays redacted
- regression coverage proving visual-context artifact bridge snippets, visual-context artifact return strings, artifact upload calls, public-access flags, bucket names, object keys, and visual artifact notes stay redacted
- regression coverage proving visual-context training-dataset bridge snippets, visual-context training return strings, training dataset calls, dataset IDs, split names, labels, screenshot bytes, and OCR text stay redacted
- regression coverage proving visual-context telemetry bridge snippets, visual-context telemetry return strings, telemetry calls, trace names, project labels, notes, screenshot bytes, and OCR text stay redacted
- regression coverage proving visual-context prompt-cache bridge snippets, visual-context prompt-cache return strings, prompt-cache calls, cache keys, notes, screenshot bytes, and OCR text stay redacted
...
- source-defined tool handler metadata for clipboard material persisted into prompt/LLM/response/semantic caches with runtime credentials and caller-selected cache scope, without emitting clipboard contents, clipboard helper calls, prompt-cache calls, cache keys, namespaces, cache values, stored payloads, retention notes, or return strings
- regression coverage proving clipboard prompt-cache bridge snippets, clipboard helper calls, clipboard variables, prompt-cache calls, cache keys, namespaces, cache values, stored payloads, and clipboard prompt-cache return strings stay redacted
- rule `AGENTCSP-TOOL-152` for source-defined tools that read desktop or browser clipboard material and persist it into prompt/LLM/response/semantic caches under runtime cache authority and caller-selected cache routing
- source-defined tool handler metadata for clipboard material published into prompt or instruction registries with runtime credentials and caller-selected prompt routing, without emitting clipboard contents, clipboard helper calls, registry calls, prompt IDs, prompt roles, prompt namespaces, prompt bodies, derived clipboard variables, or return strings
- regression coverage proving clipboard prompt-registry bridge snippets, clipboard helper calls, clipboard variables, registry calls, prompt routing, prompt bodies, and clipboard prompt-registry return strings stay redacted
- rule `AGENTCSP-TOOL-198` for source-defined tools that read desktop or browser clipboard material and publish copied text into prompt or instruction registries under runtime registry authority and caller-selected prompt routing
- source-defined tool handler metadata for clipboard material exported into training, fine-tuning, eval, feedback, or model-improvement datasets with runtime credentials and caller-selected dataset routing, without emitting clipboard contents, clipboard helper calls, dataset calls, dataset IDs, split names, source labels, training records, derived clipboard variables, or return strings
- regression coverage proving clipboard training-dataset bridge snippets, clipboard helper calls, clipboard variables, dataset calls, dataset routing, prompt/completion records, and clipboard training-dataset return strings stay redacted
- rule `AGENTCSP-TOOL-199` for source-defined tools that read desktop or browser clipboard material and export copied text into training, fine-tuning, eval, feedback, or model-improvement datasets under runtime dataset authority and caller-selected dataset routing
- source-defined tool handler metadata for clipboard material delegated to remote agents or A2A peers with runtime delegation credentials and caller-selected targets, without emitting clipboard contents, clipboard helper calls, delegated-agent calls, target URLs, target agent IDs, forwarded context, goals, notes, derived clipboard variables, or return strings
- regression coverage proving clipboard remote-agent delegation bridge snippets, clipboard helper calls, clipboard variables, delegated-agent calls, target routing, forwarded context, and clipboard delegation return strings stay redacted
- rule `AGENTCSP-TOOL-200` for source-defined tools that read desktop or browser clipboard material and delegate copied text to remote agents or A2A peers under runtime delegation authority and caller-selected agent routing
- source-defined tool handler metadata for clipboard material used as credential grant material, signing input, token templates, subjects, audiences, scopes, roles, or impersonation selectors with runtime broker credentials, without emitting clipboard contents, clipboard helper calls, credential-broker calls, grant material, issued credentials, or return strings
- regression coverage proving clipboard credential-issuance bridge snippets, clipboard helper calls, clipboard variables, credential-broker calls, grant material, subjects, audiences, and issued-credential return strings stay redacted
- rule `AGENTCSP-TOOL-201` for source-defined tools that read desktop or browser clipboard material and use copied text to mint, sign, assume, impersonate, or issue agent credentials under runtime broker authority
- source-defined tool handler metadata for clipboard material pasted into authenticated browser sessions with runtime browser credentials and caller-selected targets or selectors, without emitting clipboard contents, clipboard helper calls, browser/page/driver calls, target URLs, selectors, pasted values, or return strings
- regression coverage proving clipboard browser-automation bridge snippets, clipboard helper calls, clipboard variables, browser automation calls, target routing, selectors, and browser-paste return strings stay redacted
- rule `AGENTCSP-TOOL-202` for source-defined tools that read desktop or browser clipboard material and submit copied text through authenticated browser automation under runtime browser authority
- source-defined tool handler metadata for clipboard material exported into public or shareable artifact storage with runtime artifact credentials and caller-selected destinations, without emitting clipboard contents, clipboard helper calls, artifact storage calls, bucket names, object keys, public links, artifact payloads, or return strings
- regression coverage proving clipboard artifact bridge snippets, clipboard helper calls, clipboard variables, artifact export calls, bucket/object routing, public destination controls, and clipboard artifact return strings stay redacted
- rule `AGENTCSP-TOOL-203` for source-defined tools that read desktop or browser clipboard material and export copied text into public or shareable artifacts under runtime artifact authority
- source-defined tool handler metadata for clipboard material passed into shell or subprocess execution under runtime authority, without emitting clipboard contents, clipboard helper calls, shell calls, command arguments, working directories, execution reasons, environment values, or return strings
- regression coverage proving clipboard shell-execution bridge snippets, clipboard helper calls, clipboard variables, shell/subprocess calls, command arguments, working directories, execution reasons, and clipboard command return strings stay redacted
- rule `AGENTCSP-TOOL-196` for source-defined tools that read desktop or browser clipboard material and pass copied text into shell or subprocess execution under runtime authority
- source-defined tool handler metadata for clipboard material passed into dynamic code execution under runtime authority, without emitting clipboard contents, clipboard helper calls, eval/Function/vm/exec snippets, generated handlers, runtime scopes, environment values, or return strings
- regression coverage proving clipboard dynamic-code bridge snippets, clipboard helper calls, clipboard variables, dynamic-code calls, generated handlers, runtime scope values, and clipboard code return strings stay redacted
- rule `AGENTCSP-TOOL-197` for source-defined tools that read desktop or browser clipboard material and pass copied text into eval, Function, vm, exec, or interpreter execution under runtime authority
- source-defined tool handler metadata for external approval-channel decisions that auto-execute privileged actions with runtime approval-channel credentials and weak identity or replay controls, without emitting channel calls, approval messages, approver selectors, decision objects, executor calls, action payloads, or return strings
- regression coverage proving external approval-channel snippets, decision variables, executor calls, action payloads, customer approval context, and approval return strings stay redacted
- rule `AGENTCSP-TOOL-153` for source-defined tools that send caller/customer context into external approval channels with weak identity or replay controls and then automatically execute privileged actions from the approval response
- source-defined tool handler metadata for caller-selected local key or credential files used to mint, sign, assume, impersonate, or issue agent credentials with runtime broker credentials, without emitting file contents, file-read calls, credential paths, signing material, identity-broker calls, issued credentials, or return strings
- regression coverage proving local-file credential-issuance bridge snippets, file-read calls, identity-broker calls, local signing-key variables, local credential return strings, and local credential paths stay redacted
- rule `AGENTCSP-TOOL-154` for source-defined tools that read caller-selected local key or credential files and use file-derived material to issue agent credentials under runtime broker authority
- source-defined tool handler metadata for environment-backed signing, broker, or grant material used to mint, sign, assume, impersonate, or issue agent credentials, without emitting environment reads, key names, secret values, signing material, identity-broker calls, issued credentials, or return strings
- regression coverage proving env-secret credential-issuance bridge snippets, environment reads, identity-broker calls, env signing-key variables, and env credential return strings stay redacted
- rule `AGENTCSP-TOOL-155` for source-defined tools that use environment-derived secret material to issue agent credentials under runtime broker authority
- source-defined tool handler metadata for environment-backed auth material forwarded as headers, bearer tokens, API keys, or token fields into caller-selected outbound URLs, webhooks, or endpoints, without emitting environment reads, request calls, header construction, bearer strings, destinations, payloads, or return strings
- regression coverage proving env-secret external-write snippets, `process.env`, `os.getenv`, `fetch(`, `httpx.post`, bearer-header construction, and raw destination snippets stay redacted
- rule `AGENTCSP-TOOL-156` for source-defined tools that forward environment-derived secret material into caller-selected external writes under runtime credential authority
- source-defined tool handler metadata for environment-backed secret material forwarded into model-provider prompt, message, input, content, or instruction fields, without emitting environment reads, model SDK calls, prompt payloads, provider responses, secret-derived variables, or return strings
- regression coverage proving env-secret prompt snippets, `process.env`, `os.getenv`, runtime credential variables, model SDK calls, and env-secret prompt return strings stay redacted
- rule `AGENTCSP-TOOL-157` for source-defined tools that place environment-derived secret material into model-provider prompts under runtime model authority
- source-defined tool handler metadata for environment-backed secret material persisted into durable memory/RAG/vector/session/state stores, without emitting environment reads, memory-store calls, namespaces, keys, stored payloads, secret-derived variables, retention notes, or return strings
- regression coverage proving env-secret memory snippets, `process.env`, `os.getenv`, memory-store calls, memory credential variables, persisted secret strings, and env-secret memory return strings stay redacted
- rule `AGENTCSP-TOOL-158` for source-defined tools that persist environment-derived secret material into durable memory under runtime memory authority
- source-defined tool handler metadata for environment-backed secret material persisted into customer or operational database records with runtime database authority, without emitting env reads, key values, secret values, database calls, SQL strings, record fields, stored payloads, or return strings
- regression coverage proving env-secret database snippets, `process.env`, `os.getenv`, database calls, database credential variables, SQL strings, persisted secret strings, and env-secret database return strings stay redacted
- rule `AGENTCSP-TOOL-166` for source-defined tools that persist environment-derived secret material into database records under runtime database authority
- source-defined tool handler metadata for environment-backed secret material persisted into prompt, LLM, response, or semantic caches with runtime prompt-cache authority, without emitting env reads, key values, secret values, prompt-cache calls, cache keys, namespaces, cache values, stored payloads, TTLs, or return strings
- regression coverage proving env-secret prompt-cache snippets, `process.env`, `os.getenv`, prompt-cache calls, cache credential variables, persisted secret strings, and env-secret prompt-cache return strings stay redacted
- rule `AGENTCSP-TOOL-159` for source-defined tools that persist environment-derived secret material into prompt, LLM, response, or semantic caches under runtime prompt-cache authority
- source-defined tool handler metadata for environment-backed secret material exported into training, fine-tuning, eval, or model-improvement datasets, without emitting env reads, key values, secret values, dataset client calls, dataset IDs, split names, source labels, training records, or return strings
- regression coverage proving env-secret training-dataset snippets, `process.env`, `os.getenv`, dataset export calls, training credential variables, persisted secret strings, and env-secret training return strings stay redacted
- rule `AGENTCSP-TOOL-160` for source-defined tools that export environment-derived secret material into training, fine-tuning, eval, or model-improvement datasets under runtime training authority
- source-defined tool handler metadata for environment-backed secret material promoted into feedback, RLHF, eval, reward-model, or model-improvement pipelines with caller-selected routing, without emitting env reads, key values, secret values, feedback pipeline calls, dataset IDs, promotion routes, feedback records, reviewer notes, or return strings
- regression coverage proving env-secret feedback snippets, `process.env`, `os.getenv`, feedback promotion calls, feedback credential variables, persisted secret strings, and env-secret feedback return strings stay redacted
- rule `AGENTCSP-TOOL-167` for source-defined tools that promote environment-derived secret material through feedback, RLHF, eval, reward-model, or model-improvement pipelines under runtime feedback authority
- source-defined tool handler metadata for environment-backed secret material exported into telemetry, tracing, logging, or observability payloads, without emitting env reads, key values, secret values, telemetry calls, trace IDs, project labels, trace payloads, or return strings
- regression coverage proving env-secret telemetry snippets, `process.env`, `os.getenv`, telemetry calls, trace credential variables, persisted secret strings, and env-secret telemetry return strings stay redacted
- rule `AGENTCSP-TOOL-161` for source-defined tools that export environment-derived secret material into AI telemetry or observability traces under runtime telemetry authority
- source-defined tool handler metadata for environment-backed secret material exported into public or shareable artifact storage with runtime artifact authority and caller-selected object routing, without emitting env reads, key values, secret values, artifact calls, bucket names, object keys, public links, payloads, or return strings
- regression coverage proving env-secret artifact snippets, `process.env`, `os.getenv`, artifact upload calls, artifact credential variables, persisted secret strings, and env-secret artifact return strings stay redacted
- rule `AGENTCSP-TOOL-162` for source-defined tools that export environment-derived secret material into public or shareable artifacts under runtime artifact authority
- source-defined tool handler metadata for environment-backed secret material enqueued into background-agent or task-queue work with runtime queue authority and caller-selected routes, without emitting env reads, key values, secret values, queue calls, route names, job payloads, replay flags, or return strings
- regression coverage proving env-secret task-queue snippets, `process.env`, `os.getenv`, queue calls, queue credential variables, persisted secret strings, and env-secret queue return strings stay redacted
- rule `AGENTCSP-TOOL-163` for source-defined tools that enqueue environment-derived secret material into background-agent jobs under runtime queue authority
- source-defined tool handler metadata for environment-backed secret material forwarded into remote-agent or A2A delegated task context with runtime delegation authority and caller-selected targets, without emitting env reads, key values, secret values, delegated-agent calls, target URLs, forwarded context, bearer headers, or return strings
- regression coverage proving env-secret remote-agent delegation snippets, `process.env`, `os.getenv`, delegation calls, delegated credential variables, persisted secret strings, and env-secret delegation return strings stay redacted
- rule `AGENTCSP-TOOL-164` for source-defined tools that delegate environment-derived secret material to remote agents or A2A peers under runtime delegation authority
- source-defined tool handler metadata for environment-backed secret material inserted into authenticated browser automation with caller-selected targets or selectors, without emitting env reads, key values, secret values, browser calls, target URLs, selectors, submitted values, bearer headers, or return strings
- regression coverage proving env-secret browser-automation snippets, `process.env`, `os.getenv`, browser fill calls, browser credential variables, replay strings, and env-secret browser return strings stay redacted
- rule `AGENTCSP-TOOL-165` for source-defined tools that replay environment-derived secret material through authenticated browser automation under runtime browser authority
- source-defined tool handler metadata for environment-backed secret material used as authorization grant roles, scopes, resources, subjects, reasons, or metadata with broad or approval-free policy writes, without emitting env reads, key values, secret values, broker calls, grant payloads, tenant selectors, role values, or return strings
- regression coverage proving env-secret authorization snippets, `process.env`, `os.getenv`, broker calls, env grant variables, grant payloads, and env-secret authorization return strings stay redacted
- rule `AGENTCSP-TOOL-168` for source-defined tools that grant authorization from environment-derived secret material under runtime authorization authority
- source-defined tool handler metadata for environment-backed secret material used as guardrail, approval, moderation, or safety-policy payload material while switching controls to default-allow, approval-off, disabled, monitor-only, or fail-open posture, without emitting env reads, key values, secret values, policy calls, selected controls, policy payloads, selected modes, or return strings
- regression coverage proving env-secret safety-policy snippets, `process.env`, `os.getenv`, policy calls, env policy variables, policy payloads, and env-secret safety-policy return strings stay redacted
- rule `AGENTCSP-TOOL-169` for source-defined tools that weaken safety policy from environment-derived secret material under runtime policy authority
- source-defined tool handler metadata for environment-backed secret material published into prompt or instruction registries with runtime registry credentials and caller-selected prompt IDs, roles, namespaces, or destinations, without emitting env reads, key values, secret values, registry calls, prompt bodies, prompt selectors, or return strings
- regression coverage proving env-secret prompt-registry snippets, `process.env`, `os.getenv`, registry calls, env prompt-registry variables, prompt bodies, and env-secret prompt-registry return strings stay redacted
- rule `AGENTCSP-TOOL-170` for source-defined tools that publish environment-derived secret material into prompt or instruction registries under runtime registry authority
...
- regression coverage proving unreadable non-root directories are omitted with `SCAN_DIRECTORY_READ_FAILED` while sibling files continue to be scanned and raw OS details stay redacted
