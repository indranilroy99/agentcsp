...
`agent-manifest.json` includes `triage_summary`, a deterministic rollup of total findings, active findings, suppressions, severity, confidence, surface types, recommended controls, top rules, and top active risks. It also includes `action_plan`, a bounded prioritized remediation list with response tiers, truncation metadata, baseline status, and deterministic owner workload summaries for routing work across agent platform, runtime platform, CI/CD, identity/secrets, data/RAG, and application-security owners. Owner summaries include response-tier counts, control mix, surface mix, and bounded top action IDs. The Markdown report renders both near the top for human triage.
