...
`agent-manifest.json` includes `triage_summary`, a deterministic rollup of total findings, active findings, suppressions, severity, confidence, surface types, recommended controls, top rules, and top active risks. It also includes `action_plan`, a prioritized remediation list with deterministic owner hints for routing work across agent platform, runtime platform, CI/CD, identity/secrets, data/RAG, and application-security owners. The Markdown report renders both near the top for human triage.
