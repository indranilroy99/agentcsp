...
- TypeScript/JavaScript MCP SDK, Python/FastMCP, Python agent-framework, and JavaScript/TypeScript agent-framework source registration metadata for bounded `server.tool(...)`, `server.registerTool(...)`, `@mcp.tool(...)`, LangChain/LangGraph `@tool(...)`, `StructuredTool.from_function(...)`, OpenAI Agents `@function_tool`, CrewAI-style `@tool(...)`, AI SDK `tool(...)`, and LangChain `new DynamicStructuredTool(...)` extraction, including tool names, input field names, local Pydantic/BaseModel request-model field expansion, Zod/JSON-schema-like field maps, read-only/idempotency hints where available, open-world schema posture, and redacted source/handler boundaries without emitting handler bodies, source snippets, model class names, raw schemas, docstrings, or raw descriptions
