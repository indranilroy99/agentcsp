...
- MCP client root, sampling, and elicitation metadata for remote context requests, sensitive field categories, redaction/sanitization posture, and approval posture without emitting raw root paths, root names, sampling prompts, elicitation schemas, requested field names, or token placeholders
