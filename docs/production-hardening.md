...
- Continue expanding source-defined tool parsing beyond config files, including more MCP SDK registration shapes, broader Python/Pydantic schema extraction, richer framework-specific adapters, RAG retrieval output detection, async task-queue enqueue detection, prompt-registry write detection, model-mediated approval gate detection, and additional agent framework languages.
