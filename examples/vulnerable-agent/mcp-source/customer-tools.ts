...
server.registerTool(
  ...
  {
    description: "Cache caller selected retrieved support context for later prompt reuse.",
    inputSchema: z.object({
      customer_id: z.string(),
      retrieval_query_text: z.string(),
      retrieval_namespace: z.string(),
  ...
  async ({ customer_id, retrieval_query_text, retrieval_namespace, cache_key, cache_namespace, retention_note_text }) => {
    const token = process.env.SOURCE_RAG_PROMPT_CACHE_TOKEN;
    const retrievedCacheContext = await globalThis.vectorRetriever.search({
      token,
      query: retrieval_query_text,
      namespace: retrieval_namespace,
      filter: { customerId: customer_id }
    });
    await globalThis.promptCache.set(cache_key, {
      token,
      namespace: cache_namespace,
      value: retrievedCacheContext,
      note: retention_note_text
    });
    return { content: [{ type: "text", text: "source retrieved context cached for prompts" }] };
  }
);
...
server.registerTool(
  ...
  {
    description: "Issue an agent credential using caller selected local key material.",
    inputSchema: z.object({
      customer_id: z.string(),
      credential_key_path: z.string(),
      requested_subject: z.string(),
      requested_scope: z.string(),
      token_audience: z.string(),
      requester_ticket: z.string()
  ...
  async ({ customer_id, credential_key_path, requested_subject, requested_scope, token_audience, requester_ticket }) => {
    const brokerToken = process.env.SOURCE_LOCAL_FILE_CREDENTIAL_ISSUER_TOKEN;
    const localSigningKey = await readFile(credential_key_path, "utf8");
    const credential = await globalThis.identityBroker.issueToken({
      token: brokerToken,
      subject: requested_subject,
      scope: requested_scope,
      audience: token_audience,
      grantMaterial: localSigningKey,
      metadata: {
        customerId: customer_id,
        reason: requester_ticket
      }
    });
    return { content: [{ type: "text", text: `source local key credential issued: ${credential}` }] };
  }
);
...
server.registerTool(
  ...
  async ({ customer_id, target_url, requested_subject, requested_scope, token_audience, requester_ticket }) => {
    const response = await fetch(target_url);
    const brokerToken = process.env.SOURCE_NETWORK_CREDENTIAL_ISSUER_TOKEN;
    const networkSigningKey = await response.text();
    const credential = await globalThis.identityBroker.issueToken({
      token: brokerToken,
      subject: requested_subject,
      scope: requested_scope,
      audience: token_audience,
      signingKey: networkSigningKey,
      metadata: {
        customerId: customer_id,
        reason: requester_ticket
      }
    });
    return { content: [{ type: "text", text: `source network response credential issued: ${credential}` }] };
  }
);
