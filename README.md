...
- **Runtime Posture Inventory**: parses agent runtime configs for sandbox mode, approval policy, network access, Claude-style permission allowlists, privileged tool allowlists, MCP references, auto-approved MCP tools, broad web scopes, auto-approved package scripts, AI model endpoint egress, authenticated browser-session authority, browser remote-debugging exposure, inbound message trigger authority, multi-agent orchestration authority, live eval/red-team harness authority, disabled agent-safety controls around privileged tools, agent identity delegation, dynamic agent extension loading, self-modifying control-plane writes, model-mediated approval gates, context-composer role boundaries, artifact/output export boundaries, generic webhook/callback egress, container isolation boundaries, SaaS/API connector authority, secret-manager authority, database connector authority, AI telemetry export, and env key exposure.
