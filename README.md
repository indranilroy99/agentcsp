...
- **Runtime Posture Inventory**: parses agent runtime configs for sandbox mode, approval policy, network access, Claude-style permission allowlists, privileged tool allowlists, MCP references, auto-approved MCP tools, broad web scopes, auto-approved package scripts, AgentCSP advisory-policy integrity, public A2A/agent-card exposure, remote prompt-registry supply posture, agent package-manifest supply-chain posture, agent deployment image provenance, AI model endpoint egress, AI model router and fallback boundaries, AI embedding and indexing boundaries, authenticated browser-session authority, browser remote-debugging exposure, browser extension/profile exposure, inbound message trigger authority, multi-agent orchestration authority, live eval/red-team harness authority, AI training and fine-tuning dataset boundaries, LLM prompt/response cache replay boundaries, disabled agent-safety controls around privileged tools, agent identity delegation, cloud control-plane authority, dynamic agent extension loading, self-modifying control-plane writes, model-mediated approval gates, context-composer role boundaries, artifact/output export boundaries, generic webhook/callback egress, container isolation boundaries, code interpreter and notebook runtime authority, SaaS/API connector authority, secret-manager authority, database connector authority, AI telemetry export, and env key exposure.
